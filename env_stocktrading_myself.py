    ...
    def _take_action(self, action):
        ...
        if self.hold_stock_num == 0:
            if action > 0.5:
                ...
                self.current_reward = self.stock_balance - self.hold_stock_money
        ...
        #是持股状态
            ...
            if self.hold_stock_num > 0:
                if action > 0.5:
                    self.current_reward = self.hold_stock_money * (
                            self.df.loc[self.current_step, 'close'] - self.df.loc[self.current_step - 1, 'close'])
                    return self.current_reward
                elif action < -0.5:
                    self.stock_balance = self.hold_stock_num * self.df.loc[self.current_step, 'close']
                    self.hold_stock_num = 0
