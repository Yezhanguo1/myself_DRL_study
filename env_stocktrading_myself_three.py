...
class StockTradingEnv(gym.Env):
    ...
    def reset(self,seed=None,options=None):
        ...
        #是否持股
        self.hold_stocks = False
        #总回报
        self.total_reward_list.append(self.target_reward)
        print(f"回报列表为##################################{self.total_reward_list}")
        self.target_reward = 0
    ...
    def _take_action(self, action,transformer=False):
        if transformer:
        ...
        reward_current \
            = 100 * (self.df.loc[self.current_step, 'close']
                     - self.df.loc[self.current_step - 1, 'close']) / self.df.loc[self.current_step - 1, 'close']


        if not self.hold_stocks:
            #买
            if action > 0:
                quotient, remainder = divmod(self.current_money, self.df.loc[self.current_step-1, 'close'] * 100)
                self.hold_stock_num = quotient * 100
                self.hold_stock_money = self.hold_stock_num * self.df.loc[self.current_step-1, 'close']
                self.current_money -= self.hold_stock_money
                self.stock_balance = self.hold_stock_num * self.df.loc[self.current_step, 'close']

                self.hold_stocks = True

                return reward_current+10
            #不买
            else:
                return 0
            #卖
